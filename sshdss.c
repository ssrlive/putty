...
static bool dss_verify(ssh_key *key, ptrlen sig, ptrlen data)
{
    ...
    bool toret;
    mp_int *r, *s, *w, *sha, *u1, *u2, *gu1p, *yu2p, *gu1yu2p, *v;
    unsigned invalid;
    ...
    r = mp_from_bytes_be(make_ptrlen(sig.ptr, 20));
    s = mp_from_bytes_be(make_ptrlen((const char *)sig.ptr + 20, 20));
    if (!r || !s) {
        if (r)
            mp_free(r);
        if (s)
    ...
    sha = mp_from_bytes_be(make_ptrlen(hash, 20));
    u1 = mp_modmul(sha, w, dss->q);

    /*
     * Step 3. u2 <- r * w mod q.
     */
    u2 = mp_modmul(r, w, dss->q);

    /*
     * Step 4. v <- (g^u1 * y^u2 mod p) mod q.
     */
    gu1p = mp_modpow(dss->g, u1, dss->p);
    yu2p = mp_modpow(dss->y, u2, dss->p);
    gu1yu2p = mp_modmul(gu1p, yu2p, dss->p);
    v = mp_mod(gu1yu2p, dss->q);
                     ...
                     unsigned char *digest, int digest_len)
{
     ...
     */
    modminus2 = mp_copy(modulus);
    mp_sub_integer_into(modminus2, modminus2, 2);
    proto_k = mp_from_bytes_be(make_ptrlen(digest512, 64));
    k = mp_mod(proto_k, modminus2);
    mp_free(proto_k);
    mp_free(modminus2);
...
static void dss_sign(ssh_key *key, ptrlen data, unsigned flags, BinarySink *bs)
{
    ...
    unsigned char digest[20];
    int i;
    mp_int *k, *kinv, *gkp, *r, *hash, *xr, *hxr, *s;

    hash_simple(&ssh_sha1, data, digest);

    k = dss_gen_k("DSA deterministic k generator", dss->q, dss->x,
                          digest, sizeof(digest));
    kinv = mp_invert(k, dss->q);       /* k^-1 mod q */

    /*
     * Now we have k, so just go ahead and compute the signature.
     */
    gkp = mp_modpow(dss->g, k, dss->p); /* g^k mod p */
    r = mp_mod(gkp, dss->q);        /* r = (g^k mod p) mod q */
    mp_free(gkp);

    hash = mp_from_bytes_be(make_ptrlen(digest, 20));
    xr = mp_mul(dss->x, r);
    hxr = mp_add(xr, hash);         /* hash + x*r */
    s = mp_modmul(kinv, hxr, dss->q); /* s = k^-1 * (hash+x*r) mod q */
    mp_free(hxr);
    mp_free(xr);
    mp_free(kinv);
    mp_free(k);
